...
export class UsersService {
  ...
  async findByTg(id: number): Promise<User> {
    const user = await this.usersRepository.findOne({where: { telegramId: new TelegramId(id) }})
    if(!user) throw new NotFoundException(`User with id ${id} is not found`)
    return user;
  }

  async remove(id: number): Promise<void> {
    const result = await this.usersRepository.delete({telegramId: new TelegramId(id)})
    if(result.affected === 0) throw new NotFoundException(`User with id ${id} is not found`)
  }
}
